        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                if self.config.training.algo == "ssm":
                    ...
                    loss, *_ = sliced_score_estimation_vr(
                        scaled_score, X.detach(), n_particles=1
                    )

                elif self.config.training.algo == "dsm":
                    loss = dsm_score_estimation(
                        scaled_score, X, sigma=self.config.training.noise_std
                    )
                ...
                logging.info(
                    "step: {}, loss: {}, sigma: {}".format(step, loss.item(), sigma)
                )
                ...
                if step % 100 == 0:
                    ...
                    if self.config.training.algo == "ssm":
                        test_X += (
                            torch.randn_like(test_X) * self.config.training.noise_std
                        )
                        test_loss, *_ = sliced_score_estimation_vr(
                            scaled_score, test_X.detach(), n_particles=1
                        )
                    elif self.config.training.algo == "dsm":
                        test_loss = dsm_score_estimation(
                            scaled_score, test_X, sigma=self.config.training.noise_std
                        )
